...
export const getLatestDesignBasisRevision = async (projectId: string) => {
  const dbRevisionData = await getData(
    `${DESIGN_BASIS_REVISION_HISTORY_API}?filters=[["project_id", "=", "${projectId}"], ["status", "in", ["${DB_REVISION_STATUS.Unsubmitted}", "${DB_REVISION_STATUS.Submitted}", "${DB_REVISION_STATUS.Resubmitted}"]]]&fields=["*"]&order_by=creation asc`
  )
