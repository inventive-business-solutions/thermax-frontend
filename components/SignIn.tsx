...
import { registerBTGUser } from "actions/register"
...
export default function SignIn({ authSecret }: { authSecret: string }) {
  ...
  const onSubmit2: SubmitHandler<zod.infer<typeof createBTGUserSchema>> = async (data) => {
    setLoading(true)
    try {
      const response = await registerBTGUser(data)
  ...
  return (
    <div className="flex flex-col gap-2 rounded-xl border border-gray-300 p-6 shadow-lg">
      ...
      {watch("email") === authSecret && (
        <div className="">
          <Divider>Create BTG User</Divider>
