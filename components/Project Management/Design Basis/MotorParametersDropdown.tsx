...
export default function useMotorParametersDropdowns() {
  ...
  const { dropdownOptions: hazardousEfficiencyLevelOptions } =
    useDropdownOptions(`${HAZARDOUS_EFFICIENCY_LEVEL}?fields=["*"]`, "name");
  const { dropdownOptions: safeInsulationClassOptions } = useDropdownOptions(
    `${SAFE_INSULATION_CLASS}?fields=["*"]`,
    "name"
  );
  const { dropdownOptions: hazardousInsulationClassOptions } =
    useDropdownOptions(`${HAZARDOUS_INSULATION_CLASS}?fields=["*"]`, "name");
  const { dropdownOptions: safeTempRiseOptions } = useDropdownOptions(
    `${SAFE_TEMPERATURE_RISE}?fields=["*"]`,
    "name"
  );
  ...
  const { dropdownOptions: hazardousEnclosureIpRatingOptions } =
    useDropdownOptions(`${HAZARDOUS_ENCLOSURE_IP_RATING}?fields=["*"]`, "name");
  const { dropdownOptions: safeTerminalBoxIpRatingOptions } =
    useDropdownOptions(`${SAFE_TERMINAL_BOX_IP_RATING}?fields=["*"]`, "name");
  const { dropdownOptions: hazardousTerminalBoxIpRatingOptions } =
    useDropdownOptions(
  ...
  hazardousSpaceHeaterOptions = sortDropdownOptions(
