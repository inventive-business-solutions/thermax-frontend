...
export default function DocumentRevision() {
  ...
  const handleDownload = async (revision_id: string) => {
    ...
    const projectCreator = await getData(
      `${THERMAX_USER_API}/${project?.owner}`
    );
    const projectApprover = await getData(
      `${THERMAX_USER_API}/${project?.approver}`
    );
    ...
    for (const projectMainPkg of projectMainPackage || []) {
      ...
      const mainPkgData = await getData(
        `${PROJECT_MAIN_PKG_API}/${projectMainPkgID}`
      );
    ...
    try {
      const result = await downloadFile(
        "/method/db_revision.get_design_basis_excel",
        true,
        {
          metadata: {
      ...
      link.setAttribute(
        "download",
        `Design_Basis_${project?.project_oc_number}_R${documentRevisions[0]?.idx}.xlsx`
      ); // Filename
      ...
      render(text, record) {
        return (
          <div className="flex flex-row justify-center gap-2 hover:cursor-pointer">
            ...
            <div
              className={clsx(
                projectData?.approver !== userInfo.email && "hidden"
              )}
            >
