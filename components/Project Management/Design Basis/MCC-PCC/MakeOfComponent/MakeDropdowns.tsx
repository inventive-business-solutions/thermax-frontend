...
export default function useMakeOfComponentDropdowns() {
  ...
  const { dropdownOptions: lv_switchgearOptions } = useDropdownOptions(
    `${MAKE_LV_SWITCHGEAR}?fields=["*"]`,
    "lv_switchgear"
  )
  const { dropdownOptions: panel_enclosureOptions } = useDropdownOptions(
    `${MAKE_PANEL_ENCLOSURE}?fields=["*"]`,
    "panel_enclosure"
  )
  const { dropdownOptions: vfd_vsdOptions } = useDropdownOptions(`${MAKE_VFD_VSD}?fields=["*"]`, "vfd_vsd")
  const { dropdownOptions: soft_starterOptions } = useDropdownOptions(
