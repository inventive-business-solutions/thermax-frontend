...
const getDefaultValues = (commonConfigData: any) => {
  return {
    ...
    power_bus_main_busbar_selection:
      commonConfigData?.power_bus_main_busbar_selection || "As per IS",
    ...
    power_bus_rating_of_busbar:
      ...
      "( Min -1R X 60 mm X 12 mm) for 50 KA",
    control_bus_main_busbar_selection:
      commonConfigData?.control_bus_main_busbar_selection || "As per IS",
    ...
    control_bus_rating_of_busbar:
      commonConfigData?.control_bus_rating_of_busbar ||
      "( Min -1R X 60 mm X 12 mm) for 50 KA",
    earth_bus_main_busbar_selection:
      commonConfigData?.earth_bus_main_busbar_selection || "As per IS",
...
const CommonConfiguration = ({
  ...
  const field_motor_isolator_is_selected_controlled = watch(
    "is_field_motor_isolator_selected"
  );
  ...
  const local_push_button_station_is_Selected_controlled = watch(
    "is_local_push_button_station_selected"
  );
  ...
  const is_push_button_speed_selected_controllded = watch(
    "is_push_button_speed_selected"
  );
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (commonConfigData && commonConfigData.length > 0) {
        await updateData(
          `${COMMON_CONFIGURATION}/${commonConfigData[0].name}`,
          false,
          data
        );
