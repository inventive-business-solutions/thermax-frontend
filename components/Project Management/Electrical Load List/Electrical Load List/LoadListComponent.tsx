...
const useDataFetching = (
  ...
  }, [designBasisRevisionId, loadListLatestRevisionId, project_id]);
...
const LoadList: React.FC<LoadListProps> = ({
  ...
  const loadListOptions = useMemo(
    ...
    [loadListData, typedLoadListColumns, handleCellChange]
  );
      ...
      typedLoadListColumns.forEach((column) => {
        if (column.name === "controlScheme") {
          console.log(
            [
              ...new Set(
                selectedControlSchemeItems.filter((item: any) => item != "NA")
              ),
              "NA",
            ],
            "selectedLpbsItems"
        ...
        if (column.name === "lbpsType") {
          console.log(
            [...new Set(selectedLpbsItems), "NA"],
            "selectedLpbsItems"
          );

          column.source = [
            ...new Set(selectedLpbsItems.filter((item: any) => item != "NA")),
            "NA",
          ];
        }
      });
  ...
  const validateUniqueFeederTag = () => {
    ...
    Object.entries(duplicateValues).forEach(([_, indices]) => {
      if (indices.length > 1) {
  ...
  const handleLoadListSave = async () => {
    ...
    try {
      ...
      console.error("Error saving electrical load list:", error);
  ...
  const handleCurrentCalculation = async () => {
    ...
    const updatedLoadList: any = loadList?.map((row: any) => {
      const calculationResult = data?.find(
