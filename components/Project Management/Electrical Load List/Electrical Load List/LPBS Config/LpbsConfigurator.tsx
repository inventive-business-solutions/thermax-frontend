...
const LpbsConfigurator: React.FC<LpbsConfiguratorProps> = React.memo(({ isOpen, onClose, onConfigurationComplete }) => {
    ...
    if (storedSchemes) {
      try {
        ...
        updatedSchemes = lpbsSchemes.map((scheme) =>
          selectedItems.includes(scheme[2]) ? [true, ...scheme.slice(1)] : scheme
        )
      } catch (error) {
